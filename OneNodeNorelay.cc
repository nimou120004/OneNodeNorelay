  ...
  for (int mcs = minMcs; mcs <= maxMcs; mcs++)
    {
      uint8_t index = 0;
     // double previous = 0;
      for (int channelWidth = 40; channelWidth <= 40; )
        {
...
//                                            "ControlMode", StringValue ("VhtMcs0");
            ...
            //  positionAlloc->Add (Vector (58.0, 0.0, 0.0)); //relay 40.0 for 160MHz, 58.0 for 80MHz, 80.0 for 160MHz
              ...
              std::cout << mcs << "\t\t" << channelWidth << " MHz\t\t"  << recpack << " Packets \t\t" << (n-1) << " sources\t  "<< recpack2 << " Packets\t  " << distance << "m \t\t" << prevThroughput[0] << " throughput \t\t"<< dbs << "m\t" << recpack/((n-1)*10000) << " " << std::endl; // (n-2) - for relay, (n-1) without relay
